...
%===================================
\section{KL divergence}\label{prob:kl_div}
...
Let $P$ and $Q$ be probability distributions over $\mathbb{B} = \{\texttt{0}, \texttt{1}\}$, i.e., $p_0, p_1, q_0, q_1 \geq 0$
with $p_0 + p_1 = 1$ and $q_0 + q_1 = 1$. Write $p_x = P(x)$ and $q_x = Q(x)$. Then
\[
...
For each $t$ and $x_{<t}$, both $\mu(\cdot \mid x_{<t})$ and $\xi(\cdot \mid x_{<t})$ are probability distributions on $\mathbb{B}$ ($\xi$ by \cref{prob:one_step_ex}), so \cref{thm:pinsker} gives
\[
...
Because $\mu$ is deterministic, the outer sum over $x_{<t}$ in $s_t$ collapses
...
Let $N$ be the total number of mistakes. Summing $s_t$ over mistake steps
...
In \Cref{prob:kl} and \cref{prob:sq} we establish that the Bayesian mixture $\xi$ is
...
\begin{lemma}[Pinsker's binary inequality]\label{lem:pinsker_binary}
    ...
    \]
    using $f'(x) = \frac{p}{x} - \frac{1-p}{1-x} = \frac{p-x}{x(1-x)}$. Recall also that $x(1-x) \leq \tfrac14$ on $[0,1]$, so $\frac{1}{x(1-x)} \geq 4$.
