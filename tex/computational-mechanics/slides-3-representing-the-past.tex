...
\begin{frame}
  ...
  \vfill
  {\centering\input{figures/z1r-generator.tex}\par}
  \vfill
  {\small
...
\begin{frame}{Generalised hidden Markov models}
  ...
  \[
    ...
    \eta^{(\emptyset)}T^{(x_1)}\cdots T^{(x_n)}\phi\geq 0
      \quad\text{for every }x_{1:n}\in\mathcal X^*.
  \]
...
\begin{frame}{Two notions of minimality}
  ...
  \vfill
  \source{Park, Choe \& Veitch, \textit{The Linear Representation Hypothesis
  and the Geometry of Large Language Models} (ICML 2024); Elhage et al.,
  \textit{Toy Models of Superposition} (Anthropic, 2022).}
  \vfill
