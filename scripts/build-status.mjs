...
// A deck's label: a `% title:` in its own `%--- iliad ---` block if it has one
// (the same comment block a worksheet's main.tex opens with), else its
// \title{} (beamer's \title[short]{long} included), lightly de-TeXed; null
// when it sets neither. The override exists because several decks of one day
// often share a \title{} and differ only in \subtitle{}. The site reads the
// same thing off the staged .tex (src/lib/content.ts) — keep the two in step.
