...
- `node scripts/build-content.mjs [slug ...]` (or `./run.sh content [slug]`) —
  ...
  Converter ERRORs fail the build; warnings don't. An overfull line in the
  compiled PDF — an equation or word wider than the page — is a warning too,
  named by source `file:line`; break the line in the source rather than
  leaving it, since the web column is no wider and it escapes there as well. A worksheet that builds
...
- Every number the web *displays* (headings, theorem family, exercises) also
  ...
  converter itself.
- The injection follows `\input` (`tex2mdx/texinput.mjs`), so a worksheet split
  into `sections/*.tex` gets a labelled copy of each file
  (`sections/foo.autolabel.tex`) with main's `\input` repointed at them. The
  split is preserved rather than inlined so `main.log`'s `file:line` errors
  still name the file you actually edit. Anything else that rewrites the source
  goes through the same walk — notably the `-nosol` strip, which would
  otherwise miss every solution that lives in a section file.
